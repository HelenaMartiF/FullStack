  ...
  fetch(url) //Realiza una solicitud GET a la URL construida utilizando fetch().
    ...
    .then((data) => {
      ...
      for (const contador in country) {
        ...
        console.log(contador); //Dentro del bucle for...in, crea un objeto que contiene el nombre introducido por el usuario, el nombre del país y la probabilidad de que el nombre introducido sea de ese país.
        console.log(country[contador]);
        const objeto = {
          name: name,
          ciudad: country[contador].country_id,
